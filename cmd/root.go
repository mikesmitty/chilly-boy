...
func init() {
	...
	rootCmd.PersistentFlags().Float64("initial-cool", 50.0, "Peltier max chiller output during cooldown process (0-100)")
	rootCmd.PersistentFlags().Float64("initial-heat", 35.0, "Peltier max heater output during heat cycles (0-100)")
	rootCmd.PersistentFlags().Float64("max-cool", 100.0, "Peltier max chiller output (0-100)")
	rootCmd.PersistentFlags().Float64("max-heat", 70.0, "Peltier max heater output (0-100)")
	...
	rootCmd.PersistentFlags().Float64("pid-startup-integral", 0.0, "PID startup integral value")
	rootCmd.PersistentFlags().Float64("linear-setpoint-gain", 15.0, "Linear setpoint auto-adjust gain counteracts subtle drift over time")
	...
	rootCmd.PersistentFlags().Float64("setpoint-gain", 1.0, "Setpoint auto-adjust gain helps maintain a thin, stable layer of dew on the mirror")
	...
	rootCmd.PersistentFlags().Float64("setpoint-step-limit", 0.05, "Setpoint auto-adjust step limit, maximum setpoint value")
	rootCmd.PersistentFlags().Float64("signal-exponent", 14.0, "Exponent for signal input amplification")
	rootCmd.PersistentFlags().Float64("signal-cap", 0.0, "Cap exponential signal amplification output")
	rootCmd.PersistentFlags().Float64("target-light-ratio", 60.0, "Target light level ratio during cooldown process")
	rootCmd.PersistentFlags().Float64("target-max-light-ratio", 70.0, "Light level threshold ratio where a cooldown process is triggered")
	rootCmd.PersistentFlags().Int("mqtt-sample-interval", 10, "mqtt sample-interval for publishing sensor data")
	rootCmd.PersistentFlags().Int("pid-input-average", 1, "PID input signal averaging samples")
	rootCmd.PersistentFlags().Int("pid-output-average", 1, "PID output signal averaging samples")
