...
func init() {
	...
	rootCmd.PersistentFlags().String("pid-algo", "", "PID algorithm (classic, pessen-integral, some-overshoot, no-overshoot)")
	...
	rootCmd.PersistentFlags().Duration("pid-lp", 1000*time.Second, "PID derivative low pass filter time constant")
	...
	rootCmd.PersistentFlags().Float64("pid-ff", 0.0, "PID FeedForward Gain")
	rootCmd.PersistentFlags().Float64("pid-awg", 0.0, "PID Anti-Windup Gain")
	rootCmd.PersistentFlags().Float64("pid-max-output", 0.0, "PID heater output percentage limit")
	rootCmd.PersistentFlags().Float64("pid-min-output", 0.0, "PID cooler output percentage limit")
	rootCmd.PersistentFlags().Float64("startup-light-ratio", 80, "Startup light target as a percentage of max")
